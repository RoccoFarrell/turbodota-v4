...
export const GET: RequestHandler = async ({ params, url, setHeaders }) => {
    ...
    */
	...
	if (startDate) {
		console.log(
			`[matches][${account_id}] newest match present in query: `,
	...
	if (userResult && userResult.lastUpdated >= updateInterval && !forceFullUpdate) {
		...
		console.log(
			`[matches][${account_id}] allow update true, and user was last updated >12 hours - fetch from OD`
		);
		...
		if (d_diff && !forceFullUpdate) {
			...
			od_url = encodeURI(
				`https://api.opendota.com/api/players/${account_id}/matches?significant=0&game_mode=23`
			);
		}
		console.log(od_url);
		...
		while (txRecordCount > 0 && loopCount < 10) {
			if (txRecordCount - txBlockSize > 0) {
				...
				console.log(
					`[matches][${account_id}] - too many records, chunked insert with block size ${txBlockSize}`
				);
				console.log(
					`[matches][${account_id}] - from index ${txRecordCount - txBlockSize} to ${
						txRecordCount - 1
					}`
				);
	...
	console.log(
		`[matches][${account_id}] fetching full history after addition of d_diff matches - fetch from DB`
	);
	const matchesResult = await prisma.match.findMany({
		where: { account_id }
	});
	...
	let newResponse = new Response(
		JSON.stringify({ dataSource: dataSource, matchData: matchStats, od_url: od_url }),
		{
			headers: {
				'cache-control': `max-age=${cacheTimeoutSeconds}`
			}
