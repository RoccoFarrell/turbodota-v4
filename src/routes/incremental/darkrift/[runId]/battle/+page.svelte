<script lang="ts">
	...
	async function onContinueAfterWin() {
		try {
			...
			if (res.ok) {
				await goto(`/incremental/darkrift/${runId}`);
				return;
			}
...
<div class="w-full max-w-[1400px] mx-auto p-4 md:p-6 space-y-6">
	...
	{:else if error}
		...
		<a href="/incremental/darkrift/{runId}" class="inline-block mt-2 text-sm text-primary hover:underline"
			>← Back to run</a
		>
	{:else if battle}
		{#if battle.result !== null}
			...
			>
				...
				{:else}
					<a
						href="/incremental/darkrift/{runId}"
						class="mt-4 inline-block rounded bg-primary px-4 py-2 text-primary-foreground hover:opacity-90"
					>
						Continue
					</a>
		...
		<p class="text-sm text-gray-500 dark:text-gray-400">
			<a href="/incremental/darkrift/{runId}" class="text-primary hover:underline">← Back to run</a>
			·
