<script lang="ts">
	...
	async function chooseNextNode(nextNodeId: string) {
		...
		try {
			...
			if (enterBattle) {
				...
				await goto(`/incremental/darkrift/${runId}/battle?nodeId=${encodeURIComponent(nextNodeId)}`);
				return;
			}
	...
	$effect(() => {
		...
		if (rid && path === `/incremental/darkrift/${rid}`) {
			loadRun();
		}
	});
</script>
	...
	<div class="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-6">
		...
		{:else if error}
			<div class="rounded-xl border border-red-500/30 bg-red-950/30 p-6 text-center space-y-3">
				...
				<a href="/incremental/lineup" class="text-sm text-violet-400 hover:text-violet-300">&larr; Back to Lineups</a>
			</div>
		{:else if runState}
			<!-- Header bar -->
			<div class="flex flex-wrap items-center justify-between gap-4">
				<div class="flex items-center gap-3">
					<a href="/incremental/darkrift" class="text-gray-500 hover:text-gray-300 transition-colors text-sm">&larr;</a>
											...
											{#each previewEnemies as { enemyDefId, def, count }}
												<span class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs bg-gray-800 text-gray-400 border border-gray-700/50">
													{#if getEnemySpriteConfig(enemyDefId)?.staticImageSrc}
														...
														/>
													{:else}
														<span class="w-5 h-5 rounded bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-500 shrink-0">{(def?.name ?? enemyDefId).charAt(0)}</span>
													{/if}
									...
									<div class="flex flex-col gap-1 min-w-[140px] shrink-0">
										<div class="flex items-center gap-2">
											...
											{#if node.nodeType === 'BOSS'}
												<span class="text-[10px] px-1.5 py-0.5 rounded bg-red-500/15 border border-red-500/25 text-red-400 uppercase tracking-wider font-semibold">Boss</span>
											{:else if node.nodeType === 'ELITE'}
												<span class="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/15 border border-amber-500/25 text-amber-400 uppercase tracking-wider font-semibold">Elite</span>
											{/if}
										</div>
										{#if runState.nodeClearances?.[node.id]}
			...
			<div class="flex items-center gap-2 text-sm text-gray-600">
				<a href="/incremental/darkrift" class="text-violet-400 hover:text-violet-300 transition-colors">Dark Rift</a>
				<span>&middot;</span>
				<a href="/incremental/lineup" class="text-violet-400 hover:text-violet-300 transition-colors">Lineups</a>
				<span>&middot;</span>
				<a href="/incremental" class="text-violet-400 hover:text-violet-300 transition-colors">Dashboard</a>
			</div>
		{/if}
	</div>
</div>
