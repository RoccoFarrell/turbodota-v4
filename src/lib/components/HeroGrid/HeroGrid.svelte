<script lang="ts">
	import type { Hero, Session } from '@prisma/client';
		...
		> -->
			<div class="my-2">
				...
				{#if $herogridStore.bannedHeroes.length > 0}
					...
					<!-- <button class="btn bg-red-500 w-1/2 my-4" on:click={() => herogridStore.setBanList()}>Clear</button> -->
				{:else}
					<p>none</p>
				{/if}
			</div>
		...
		<div class="w-fullmax-w-[90%] mx-auto p-4">
			...
			<div class="grid grid-cols-2">
				...
				<div>
					...
					<!-- <p>
					{$herogridStore.bannedHeroes.length < $herogridStore.freeBans
						? $herogridStore.freeBans - $herogridStore.bannedHeroes.length
						: 0}
				</p> -->
