...
- **Decision**: Single file `src/lib/incremental/types.ts` at root; subfolders `constants/`, `engine/`, `stats/` only. (Match codebase patterns / what makes sense for the project.)
...
- **Decision**: Minimal frontend (`+layout.svelte`, `+page.svelte` for `/incremental`) plus a simple API placeholder to reserve the `api/incremental` prefix (e.g. health check).
...
- **Decision**: No Vitest config change; add a placeholder test under `src/lib/incremental/` and verify `npm run test` runs it.
...
- **Decision**: (a) Do not import battler-specific modules. Allowed: shared `@lib/utils`, `config`, Prisma (later), auth (later).
...
- **Decision**: `result: 'win' | 'lose' | null`; no separate type alias for now.
...
- **Decision**: `HeroDef.abilityIds: string[]` (length 1 now, up to 3 later).
...
- **Decision**: (c) `BattleState.enemyFocusedIndex: number` (default 0).
...
- **Decision**: `buffs` on `HeroInstance` (e.g. `buffs: { id: string }[]`); optional on `EnemyInstance` for symmetry.
...
- **Decision**: Phase 0 types use `elapsedTime: number` (seconds since battle start).
...
- **Decision**: Include in Phase 0: `NodeType`, `RunStatus`, and minimal `MapNode` / `RunState` so API and run flow can use them.
...
- [x] 1. Folder layout: `types.ts` at root vs `types/` folder – decided.
- [x] 2. Route placeholders: what to add under `routes/incremental` and `api/incremental` – decided.
- [x] 3. Vitest: any config change or only add placeholder test – decided.
- [x] 4. Isolation: definition of “card-battler” and allowed shared imports – decided.
- [x] 5. BattleResult / result type – decided.
- [x] 6. HeroDef ability id single vs array – decided.
- [x] 7. Enemy focus: how and where stored (enemyFocusedIndex) – decided.
- [x] 8. Buffs on HeroInstance/EnemyInstance – shape or defer – decided.
- [x] 9. elapsedTime vs lastTickAt for battle state – decided.
- [x] 10. Run/map types in Phase 0 vs Phase 4 – decided.
